...
def draw_pacman(screen, pos,direction_to_point): 
	...
	if counter_to_maintain_pacman_animation%4==0 :
		screen.blit(pygame.transform.rotate(pygame.transform.scale(myimage, (30, 30)),direction_to_point), pixels)
	if counter_to_maintain_pacman_animation%4==1 :
		screen.blit(pygame.transform.rotate(pygame.transform.scale(myimage1, (30, 30)),direction_to_point), pixels)
	if counter_to_maintain_pacman_animation%4==2 :
		screen.blit(pygame.transform.rotate(pygame.transform.scale(myimage2, (30, 30)),direction_to_point), pixels)
	if counter_to_maintain_pacman_animation%4==3 :
		screen.blit(pygame.transform.rotate(pygame.transform.scale(myimage3, (30, 30)),direction_to_point), pixels)			
...
myimage = pygame.image.load("pacman1.gif")
myimage1 = pygame.image.load("pacman2.gif")
myimage2 = pygame.image.load("pacman3.gif")
myimage3 = pygame.image.load("pacman4.gif")
ghostimage=pygame.image.load("ghost1.png")
ghostimage2=pygame.image.load("ghost2.png")
ghostimage3=pygame.image.load("ghost3.png")


screen = pygame.display.set_mode((800,640), 0, 32) #Width and height of the window
...
while True:
	...
	if(current_score==182 or cheat_code==5):
		...
		time_surface = bigfont.render( for_displaying_time_elapsed() , False, (255, 255, 255)) 
   ...
   #In the board, '.' denote the empty space, 'w' are the walls, 'c' are the coins
	...
	if(pacman_position==ghost1_position or pacman_position==ghost2_position or pacman_position==ghost3_position):
		...
		livessurface=bigfont.render( "You Lost a Life" , False, (255, 255, 255))
		screen.blit(livessurface,(300,300))
		pygame.display.update()
	...
	if (layout[pacman_position[1]][pacman_position[0]]=='w'):# or layout[floor(pacman_position[1])][ceil(pacman_position[0])]=='w') : #movig along column
		pacman_position=previous_pacman_position
	'''
	...
	if(layout[round(ghost1_position[1])][round(ghost1_position[0])]=='w' ):
		...
		move_direction_of_ghost1=change_the_direction_of_ghost(move_direction_of_ghost1)

	if(layout[round(ghost2_position[1])][round(ghost2_position[0])]=='w' ):
		ghost2_position=previous_ghost2_position
		move_direction_of_ghost2=change_the_direction_of_ghost(move_direction_of_ghost2)

	if(layout[round(ghost3_position[1])][round(ghost3_position[0])]=='w' ):
		ghost3_position=previous_ghost3_position
		move_direction_of_ghost3=change_the_direction_of_ghost(move_direction_of_ghost3)		
	...
	if (layout[int(pacman_position[1])][int(pacman_position[0])]=='c' ) :
		...
		layout[int(pacman_position[1])][int(pacman_position[0])]='.'				#Removing the coin
	...
	time_surface = font.render( for_displaying_time_elapsed() , False, (255, 255, 255)) 
	screen.blit(time_surface,(620,300))

	the_score_to_be_displayed= "Your Score = "+str(current_score)
	textsurface = font.render( the_score_to_be_displayed , False, (255, 255, 255))
